...
import com.cn.tfb.util.AndroidVersionCheckUtils;
import com.cn.tfb.util.ExternalOverFroyoUtils;
import com.cn.tfb.util.ExternalUnderFroyoUtils;
...
public class PrintToFileLogger implements ILogger
{
	...
	public void open()
	{
		...
		{
			logDir = ExternalOverFroyoUtils.getDiskCacheDir(AppApplication
					.getInstance().getApplicationContext(), LOG_DIR);
		}
		else
		{
			logDir = ExternalUnderFroyoUtils.getDiskCacheDir(AppApplication
					.getInstance().getApplicationContext(), LOG_DIR);
		}
		if (!logDir.exists())
		{
