...
import static com.cn.tfb.json.util.IOUtils.replaceChars;
...
public final class SerializeWriter extends Writer
{
	...
	{
		if (this.writer != null) { throw new UnsupportedOperationException(
				"writer not null"); }
		...
		if (newcount > buf.length)
		{
			if (writer != null)
			{
				...
				for (int s = 0; s < eLen;)
				{
					...
					// attension to sign.
					int i = (bytes[s++] & 0xff) << 16
							| (bytes[s++] & 0xff) << 8 | (bytes[s++] & 0xff);
				...
				if (left > 0)
				{
					...
					int i = ((bytes[eLen] & 0xff) << 10)
							| (left == 2 ? ((bytes[bytesLen - 1] & 0xff) << 2)
									: 0);
			...
			if (specialCount > 0)
			{
				...
				if (specialCount == 1)
				{
					if (lastSpecial == '\u2028')
					{
						...
						System.arraycopy(buf, srcPos, buf, destPos,
								LengthOfCopy);
	...
	public void writeFieldValue(char seperator, String name, long value)
	{
		if (value == Long.MIN_VALUE
				|| (!isEnabled(SerializerFeature.QuoteFieldNames)))
		{
		...
		int intSize = (value < 0) ? IOUtils.stringSize(-value) + 1 : IOUtils
				.stringSize(value);
	...
	public void writeFieldValue(char seperator, String name, String value)
	{
		if (isEnabled(SerializerFeature.QuoteFieldNames))
		{
			...
			else
			{
				...
				{
					writeFieldValueStringWithDoubleQuote(seperator, name,
							value, true);
				}
			}
		}
	...
	private void writeFieldValueStringWithDoubleQuote(char seperator,
			String name, String value, boolean checkSpecial)
	{
		int nameLen = name.length();
		int valueLen;
		...
		if (checkSpecial
				&& !isEnabled(SerializerFeature.DisableCheckSpecialChar))
		{
			int specialCount = 0;
			int lastSpecialIndex = -1;
			int firstSpecialIndex = -1;
	...
	final static boolean isSpecial(char ch, int features)
	{
		...
		if (ch == '/'
				&& SerializerFeature.isEnabled(features,
						SerializerFeature.WriteSlashAsSpecial)) { return true; }

		if (ch > '#' && ch != '\\') { return false; }

		if (ch <= 0x1F || ch == '\\' || ch == '"') { return true; }
