...
public class JavaBeanSerializer implements ObjectSerializer
{
	...
	public JavaBeanSerializer(Class<?> clazz, Map<String, String> aliasMap)
	{
...

		{
			...
			getters = getterList
					.toArray(new FieldSerializer[getterList.size()]);
		}
		{
			List<FieldSerializer> getterList = new ArrayList<FieldSerializer>();
			List<FieldInfo> fieldInfoList = TypeUtils.computeGetters(clazz,
			...
			sortedGetters = getterList.toArray(new FieldSerializer[getterList
					.size()]);
		}
	}
			...
			Object fieldName, Type fieldType) throws IOException
	{
		...
		if (writeReference(serializer, object)) { return; }
		...
		try
		{
			...
			if (getters.length > 0
					&& out.isEnabled(SerializerFeature.PrettyFormat))
			{
			...
			char newSeperator = FilterUtils.writeBefore(serializer, object,
					seperator);
			...
			for (int i = 0; i < getters.length; ++i)
			{
				...
				if (!FilterUtils.applyName(serializer, object,
				...
				if (!FilterUtils.apply(serializer, object,
						fieldSerializer.getName(), propertyValue))
				{
					continue;
				}

				String key = FilterUtils.processKey(serializer, object,
						fieldSerializer.getName(), propertyValue);

				Object originalValue = propertyValue;
				propertyValue = FilterUtils.processValue(serializer, object,
						fieldSerializer.getName(), propertyValue);
				...
				if (propertyValue != null
						&& serializer
								.isEnabled(SerializerFeature.NotWriteDefaultValue))
				{
					Class<?> fieldCLass = fieldSerializer.fieldInfo
							.getFieldClass();
	...
	{
		{
			...
			if (context != null
					&& context
							.isEnabled(SerializerFeature.DisableCircularReferenceDetect)) { return false; }
		}

		if (!serializer.containsReference(object)) { return false; }
	...
	public boolean isWriteAsArray(JSONSerializer serializer)
	{
		if (SerializerFeature
				.isEnabled(features, SerializerFeature.BeanToArray)) { return true; }
