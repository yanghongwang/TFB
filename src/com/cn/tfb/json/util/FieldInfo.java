...
public class FieldInfo implements Comparable<FieldInfo>
{
	...
	public FieldInfo(String name, Class<?> declaringClass, Class<?> fieldClass,
			Type fieldType, Field field, int ordinal, int serialzeFeatures)
	{
		this.name = name;
	...
	public FieldInfo(String name, Method method, Field field, Class<?> clazz,
			Type type, int ordinal, int serialzeFeatures)
	{
		this.name = name;
		this.method = method;
		this.field = field;
	...
	public static Type getFieldType(Class<?> clazz, Type type, Type fieldType)
	{
		...
		if (fieldType instanceof GenericArrayType)
		{
			...
			{
				Type fieldTypeX = Array.newInstance(
						TypeUtils.getClass(componentTypeX), 0).getClass();
				return fieldTypeX;
			}
		...
		if (!TypeUtils.isGenericParamType(type)) { return fieldType; }

		if (fieldType instanceof TypeVariable)
		{
			ParameterizedType paramType = (ParameterizedType) TypeUtils
			...
			{
				if (parameterizedClass.getTypeParameters()[i].getName().equals(
						typeVar.getName()))
				{
			...
			for (int i = 0; i < arguments.length; ++i)
			{
				...
				if (feildTypeArguement instanceof TypeVariable)
				{
					...
					if (type instanceof ParameterizedType)
					{
						...
						{
							if (clazz.getTypeParameters()[j].getName().equals(
									typeVar.getName()))
							{
	...
	{
		if (this.ordinal < o.ordinal) { return -1; }
