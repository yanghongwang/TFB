...
import com.cn.tfb.json.annotation.JSONCreator;
...
public class DeserializeBeanInfo
{
	...
	public static DeserializeBeanInfo computeSetters(Class<?> clazz, Type type)
	{
		...
		if (defaultConstructor != null)
		{
			...
			beanInfo.setDefaultConstructor(defaultConstructor);
				...
				&& !(clazz.isInterface() || Modifier.isAbstract(clazz
						.getModifiers())))
		{
			...
			if (creatorConstructor != null)
			{
				...
				{
					Annotation[] paramAnnotations = creatorConstructor
					...
					if (fieldAnnotation == null) { throw new JSONException(
							"illegal json creator"); }

					Class<?> fieldClass = creatorConstructor
							.getParameterTypes()[i];
					Type fieldType = creatorConstructor
							.getGenericParameterTypes()[i];
					Field field = TypeUtils.getField(clazz,
							fieldAnnotation.name());
					final int ordinal = fieldAnnotation.ordinal();
					final int serialzeFeatures = SerializerFeature
							.of(fieldAnnotation.serialzeFeatures());
					FieldInfo fieldInfo = new FieldInfo(fieldAnnotation.name(),
							clazz, fieldClass, fieldType, field, ordinal,
							serialzeFeatures);
			...
			if (factoryMethod != null)
			{
				...
				{
					Annotation[] paramAnnotations = factoryMethod
							.getParameterAnnotations()[i];
					JSONField fieldAnnotation = null;
					for (Annotation paramAnnotation : paramAnnotations)
					{
		...
		for (Method method : clazz.getMethods())
		{
			...
			// support builder set
			if (!(method.getReturnType().equals(Void.TYPE) || method
					.getReturnType().equals(clazz)))
			{
				continue;
			}
			...
			if (annotation != null)
			{
				...
				if (annotation.name().length() != 0)
				{
					...
					beanInfo.add(new FieldInfo(propertyName, method, null,
							clazz, type, ordinal, serialzeFeatures));
			...
			if (Character.isUpperCase(c3))
			{
				...
				{
					propertyName = TypeUtils.decapitalize(methodName
							.substring(3));
				}
				else
				{
					...
					&& !Modifier.isStatic(clazz.getModifiers()))
			{
						...
						.getDeclaredConstructors())
				{
					if (constructor.getParameterTypes().length == 1
							&& constructor.getParameterTypes()[0].equals(clazz
									.getDeclaringClass()))
					{
						defaultConstructor = constructor;
						break;
	...
	public static Constructor<?> getCreatorConstructor(Class<?> clazz)
	{
		...
		for (Constructor<?> constructor : clazz.getDeclaredConstructors())
		{
			JSONCreator annotation = constructor
					.getAnnotation(JSONCreator.class);
			if (annotation != null)
			{
				if (creatorConstructor != null) { throw new JSONException(
						"multi-json creator"); }
	...
	public static Method getFactoryMethod(Class<?> clazz)
	{
		...
		for (Method method : clazz.getDeclaredMethods())
		{
			...
			if (annotation != null)
			{
				if (factoryMethod != null) { throw new JSONException(
						"multi-json creator"); }
