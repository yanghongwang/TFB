...
public class MapDeserializer implements ObjectDeserializer
{
	...
	@SuppressWarnings(
	{ "rawtypes", "unchecked" })
	protected Object deserialze(DefaultJSONParser parser, Type type,
			Object fieldName, Map map)
	{
		if (type instanceof ParameterizedType)
		{
			...
			{
				return parseMap(parser, (Map<String, Object>) map, valueType,
						fieldName);
			}
			else
			{
	...
	public static Map parseMap(DefaultJSONParser parser,
			Map<String, Object> map, Type valueType, Object fieldName)
	{
		JSONLexer lexer = parser.getLexer();

		if (lexer.token() != JSONToken.LBRACE) { throw new JSONException(
				"syntax error, expect {, actual " + lexer.token()); }
		...
		try
		{
			for (;;)
			{
				...
				{
					if (!parser.isEnabled(Feature.AllowSingleQuotes)) { throw new JSONException(
							"syntax error"); }
				...
				{
					if (!parser.isEnabled(Feature.AllowUnQuotedFieldNames)) { throw new JSONException(
							"syntax error"); }
				...
				if (tok == JSONToken.EOF || tok == JSONToken.RBRACKET) { return map; }
			...
			Object fieldName)
	{
		...
		try
		{
			for (;;)
			{
				...
				if (lexer.token() != JSONToken.COLON) { throw new JSONException(
						"syntax error, expect :, actual " + lexer.token()); }
	...
	protected Map<Object, Object> createMap(Type type)
	{
		...
		if (type == Map.class || type == HashMap.class) { return new HashMap(); }
		...
		if (clazz.isInterface()) { throw new JSONException("unsupport type "
				+ type); }
