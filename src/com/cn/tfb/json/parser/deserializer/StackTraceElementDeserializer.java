...
public class StackTraceElementDeserializer implements ObjectDeserializer
{
			...
			Object fieldName)
	{
		...
		for (;;)
		{
			...
			else if (key == JSON.DEFAULT_TYPE_KEY)
			{
				if (lexer.token() == JSONToken.LITERAL_STRING)
				{
					...
					if (!elementType.equals("java.lang.StackTraceElement")) { throw new JSONException(
							"syntax error : " + elementType); }
				}
				else
				{
					if (lexer.token() != JSONToken.NULL) { throw new JSONException(
							"syntax error"); }
				}
			}
			else
		...
		return (T) new StackTraceElement(declaringClass, methodName, fileName,
				lineNumber);
	}
