...
public class ArrayDeserializer implements ObjectDeserializer
{
		...
		if (type instanceof GenericArrayType)
		{
			...
			if (componentType instanceof TypeVariable)
			{
				...
				if (objType instanceof ParameterizedType)
				{
					...
					{
						TypeVariable[] objTypeParams = ((Class) objRawType)
								.getTypeParameters();
						for (int i = 0; i < objTypeParams.length; ++i)
						{
							if (objTypeParams[i].getName().equals(
									typeVar.getName()))
							{
								actualType = objParamType
	...
	private <T> T toObjectArray(DefaultJSONParser parser,
			Class<?> componentType, JSONArray array)
	{
		...
		for (int i = 0; i < size; ++i)
		{
			...
			if (componentType.isArray())
			{
				...
				{
					element = toObjectArray(parser, componentType,
							(JSONArray) value);
				}
			...
			else
			{
				...
				if (element == null)
				{
					element = TypeUtils.cast(value, componentType,
							parser.getConfig());
				}
				Array.set(objArray, i, element);
