...
public class ArrayListTypeFieldDeserializer extends FieldDeserializer
{
			...
			FieldInfo fieldInfo)
	{
		...
		{
			this.itemType = ((ParameterizedType) getFieldType())
	...
	@SuppressWarnings(
	{ "unchecked", "rawtypes" })
	public final void parseArray(DefaultJSONParser parser, Type objectType,
			Collection array)
	{
				...
				&& objectType instanceof ParameterizedType)
		{
			...
			if (paramIndex != -1)
			{
				...
				{
					itemTypeDeser = parser.getConfig()
		...
		{
			String errorMessage = "exepct '[', but "
		...
		{
			itemTypeDeser = deserializer = parser.getConfig().getDeserializer(
					itemType);
