...
import com.cn.tfb.json.serializer.FilterUtils;
import com.cn.tfb.json.util.DeserializeBeanInfo;
...
public class JavaBeanDeserializer implements ObjectDeserializer
{
	...
	public JavaBeanDeserializer(ParserConfig config, Class<?> clazz, Type type)
	{
		...
		{
			FieldDeserializer fieldDeserializer = feildDeserializerMap
					.get(fieldInfo.getName().intern());
	...
	public Object createInstance(DefaultJSONParser parser, Type type)
	{
		...
		if (beanInfo.getDefaultConstructor() == null) { return null; }
		...
		{
			throw new JSONException("create instance error, class "
					+ clazz.getName(), e);
		}
	...
	@SuppressWarnings(
	{ "unchecked" })
	public <T> T deserialzeArrayMapping(DefaultJSONParser parser, Type type,
			Object fieldName, Object object)
	{
		...
		for (int i = 0; i < size; ++i)
		{
			...
			{
				Enum<?> value = lexer.scanEnum(fieldClass,
						parser.getSymbolTable(), seperator);
				fieldDeser.setValue(object, value);
			}
			else
			{
			...
			Object fieldName, Object object)
	{
		...
		try
		{
			...
			if (lexer.token() == JSONToken.LBRACKET
					&& isSupportArrayToBean(lexer)) { return deserialzeArrayMapping(
					parser, type, fieldName, object); }
			...
			for (;;)
			{
				...
				if (JSON.DEFAULT_TYPE_KEY == key)
				{
					...
					if (lexer.token() == JSONToken.LITERAL_STRING)
					{
						...
						if (type instanceof Class
								&& typeName.equals(((Class<?>) type).getName()))
						{
							if (lexer.token() == JSONToken.RBRACE)
							{
								lexer.nextToken();
						...
						ObjectDeserializer deserizer = parser.getConfig()
								.getDeserializer(userType);
						return (T) deserizer.deserialze(parser, userType,
								fieldName);
					}
					else
					{
				...
				if (lexer.token() == JSONToken.IDENTIFIER
						|| lexer.token() == JSONToken.ERROR) { throw new JSONException(
	...
	public boolean parseField(DefaultJSONParser parser, String key,
			Object object, Type objectType, Map<String, Object> fieldValues)
	{
	...
	{
		final JSONLexer lexer = parser.getLexer(); // xxx
		if (!lexer.isEnabled(Feature.IgnoreNotMatch)) { throw new JSONException(
				"setter not found, class " + clazz.getName() + ", property "
						+ key); }
	...
	{
		return Feature.isEnabled(beanInfo.getParserFeatures(),
