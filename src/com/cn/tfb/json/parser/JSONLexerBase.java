...
import static com.cn.tfb.json.parser.JSONToken.COLON;
import static com.cn.tfb.json.parser.JSONToken.COMMA;
...
import static com.cn.tfb.json.parser.JSONToken.LPAREN;
import static com.cn.tfb.json.parser.JSONToken.RBRACE;
import static com.cn.tfb.json.parser.JSONToken.RBRACKET;
import static com.cn.tfb.json.parser.JSONToken.RPAREN;
...
public abstract class JSONLexerBase implements JSONLexer, Closeable
{
	...
	public final void nextToken()
	{
		...
		for (;;)
		{
			...
			default:
				if (isEOF())
				{ // JLS
					if (token == EOF) { throw new JSONException("EOF error"); }
	...
	{
		if (this.isEnabled(Feature.InitStringFieldAsEmpty)) { return ""; }
		return null;
	}
	...
	protected abstract void arrayCopy(int srcPos, char[] dest, int destPos,
			int length);
			...
			final char quote)
	{
		...
		for (;;)
		{
			...
			if (chLocal == EOI) { throw new JSONException("unclosed.str"); }
	...
	public final String scanSymbolUnQuoted(final SymbolTable symbolTable)
	{
		...
		final boolean firstFlag = ch >= firstIdentifierFlags.length
				|| firstIdentifierFlags[first];
		if (!firstFlag) { throw new JSONException("illegal identifier : " + ch); }
		...
		if (sp == 4 && hash == NULL_HASH && charAt(np) == 'n'
				&& charAt(np + 1) == 'u' && charAt(np + 2) == 'l'
				&& charAt(np + 3) == 'l') { return null; }
	...
	public final void scanString()
	{
		...
		for (;;)
		{
			...
			if (ch == EOI) { throw new JSONException("unclosed string : " + ch); }
	...
	public final boolean isRef()
	{
		...
		return charAt(np + 1) == '$' && charAt(np + 2) == 'r'
	...
	public abstract String addSymbol(int offset, int len, int hash,
	...
	public String scanString(char expectNextChar)
	{
		...
		final String strVal;
		{
			...
			if (endIndex == -1) { throw new JSONException("unclosed str"); }
	...
	@SuppressWarnings(
	{ "unchecked", "rawtypes" })
	public Enum<?> scanEnum(Class<?> enumClass, final SymbolTable symbolTable,
			char serperator)
	{
	...
	{
		if (ch != 't') { throw new JSONException("error parse true"); }
		next();

		if (ch != 'r') { throw new JSONException("error parse true"); }
		next();

		if (ch != 'u') { throw new JSONException("error parse true"); }
		next();

		if (ch != 'e') { throw new JSONException("error parse true"); }
		next();
	...
	public final void scanTreeSet()
	{
		...
		if (ch != 'S') { throw new JSONException("error parse true"); }
		next();
	...
	{
		if (ch != 'n') { throw new JSONException("error parse null or new"); }
		next();

		if (ch == 'u')
		{
			next();
			if (ch != 'l') { throw new JSONException("error parse true"); }
			next();
		...
		if (ch != 'e') { throw new JSONException("error parse e"); }
		next();

		if (ch != 'w') { throw new JSONException("error parse w"); }
		next();
	...
	{
		if (ch != 'u') { throw new JSONException("error parse false"); }
		next();

		if (ch != 'n') { throw new JSONException("error parse false"); }
		next();

		if (ch != 'd') { throw new JSONException("error parse false"); }
		next();

		if (ch != 'e') { throw new JSONException("error parse false"); }
		next();

		if (ch != 'f') { throw new JSONException("error parse false"); }
		next();

		if (ch != 'i') { throw new JSONException("error parse false"); }
		next();
	...
	public final void scanFalse()
	{
		...
		if (ch != 'a') { throw new JSONException("error parse false"); }
		next();

		if (ch != 'l') { throw new JSONException("error parse false"); }
		next();

		if (ch != 's') { throw new JSONException("error parse false"); }
		next();
	...
	public final boolean isBlankInput()
	{
		for (int i = 0;; ++i)
		{
			...
			if (!isWhitespace(chLocal)) { return false; }
		}
	...
	private final void scanStringSingleQuote()
	{
		...
		for (;;)
		{
			...
			if (chLocal == EOI) { throw new JSONException(
	...
	public final Number decimalValue(boolean decimal)
	{
		...
		if (chLocal == 'F') { return Float.parseFloat(numberString());
		// return Float.parseFloat(new String(buf, np, sp - 1));
		}

		if (chLocal == 'D') { return Double.parseDouble(numberString());
