...
import static com.cn.tfb.json.parser.JSONLexer.EOI;
import static com.cn.tfb.json.parser.JSONToken.EOF;
import static com.cn.tfb.json.parser.JSONToken.ERROR;
import static com.cn.tfb.json.parser.JSONToken.FALSE;
import static com.cn.tfb.json.parser.JSONToken.LBRACE;
import static com.cn.tfb.json.parser.JSONToken.LBRACKET;
import static com.cn.tfb.json.parser.JSONToken.LITERAL_FLOAT;
import static com.cn.tfb.json.parser.JSONToken.LITERAL_INT;
import static com.cn.tfb.json.parser.JSONToken.LITERAL_STRING;
import static com.cn.tfb.json.parser.JSONToken.NEW;
import static com.cn.tfb.json.parser.JSONToken.NULL;
import static com.cn.tfb.json.parser.JSONToken.RBRACKET;
import static com.cn.tfb.json.parser.JSONToken.SET;
import static com.cn.tfb.json.parser.JSONToken.TREE_SET;
import static com.cn.tfb.json.parser.JSONToken.TRUE;
import static com.cn.tfb.json.parser.JSONToken.UNDEFINED;
...
import com.cn.tfb.json.parser.deserializer.CollectionResolveFieldDeserializer;
import com.cn.tfb.json.parser.deserializer.ExtraProcessor;
import com.cn.tfb.json.parser.deserializer.ExtraTypeProvider;
...
import com.cn.tfb.json.parser.deserializer.ListResolveFieldDeserializer;
import com.cn.tfb.json.parser.deserializer.MapResolveFieldDeserializer;
...
public class DefaultJSONParser extends AbstractJSONParser implements Closeable
{
	...
	public DefaultJSONParser(final char[] input, int length,
			final ParserConfig config, int features)
	{
	...
	public DefaultJSONParser(final Object input, final JSONLexer lexer,
	...
	public final Object parseObject(final Map object, Object fieldName)
	{
		...
		if (lexer.token() != JSONToken.LBRACE
				&& lexer.token() != JSONToken.COMMA) { throw new JSONException(
				"syntax error, expect {, actual " + lexer.tokenName()); }
		...
		try
		{
			...
			for (;;)
			{
				...
				if (ch == '"')
				{
					...
					lexer.skipWhitespace();
					ch = lexer.getCurrent();
					if (ch != ':') { throw new JSONException("expect ':' at "
							+ lexer.pos() + ", name " + key); }
				}
				else if (ch == '}')
				{
				...
				{
					if (!isEnabled(Feature.AllowSingleQuotes)) { throw new JSONException(
							"syntax error"); }
				...
				{
					if (!isEnabled(Feature.AllowUnQuotedFieldNames)) { throw new JSONException(
							"syntax error"); }
				...
				if (key == JSON.DEFAULT_TYPE_KEY)
				{
					...
					if (lexer.token() == JSONToken.RBRACE)
					{
						...
						try
						{
							...
							ObjectDeserializer deserializer = this.config
									.getDeserializer(clazz);
							if (deserializer instanceof JavaBeanDeserializer)
							{
								instance = ((JavaBeanDeserializer) deserializer)
										.createInstance(this, clazz);
							}
					...
					if (lexer.token() == JSONToken.LITERAL_STRING)
					{
						...
						if ("@".equals(ref))
						{
							if (this.getContext() != null)
							{
								...
								{
									refValue = thisContext.getParentContext()
											.getObject();
								}
							}
						}
						else if ("..".equals(ref))
						{
							...
							{
								addResolveTask(new ResolveTask(parentContext,
										ref));
						...
						if (lexer.token() != JSONToken.RBRACE) { throw new JSONException(
								"syntax error"); }
						lexer.nextToken(JSONToken.COMMA);
				...
				else if (ch >= '0' && ch <= '9' || ch == '-')
				{
					...
					{
						value = lexer
								.decimalValue(isEnabled(Feature.UseBigDecimal));
					}
				...
				else
				{
					...
					{
						throw new JSONException("syntax error, position at "
								+ lexer.pos() + ", name " + key);
					}
				}
	...
	public void parseArray(Type type, Collection array, Object fieldName)
	{
		if (lexer.token() == JSONToken.SET
				|| lexer.token() == JSONToken.TREE_SET)
		{
			lexer.nextToken();
		}

		if (lexer.token() != JSONToken.LBRACKET) { throw new JSONException(
				"exepct '[', but " + JSONToken.name(lexer.token())); }
		...
		try
		{
			for (int i = 0;; ++i)
			{
				...
				if (int.class == type)
				{
					Object val = IntegerCodec.instance.deserialze(this, null,
							null);
					array.add(val);
				}
				else if (String.class == type)
	...
	public Object[] parseArray(Type[] types)
	{
		...
		if (types.length == 0)
		{
			...
			if (lexer.token() != JSONToken.RBRACKET) { throw new JSONException(
					"syntax error"); }
		...
		for (int i = 0; i < types.length; ++i)
		{
			...
			else
			{
				...
				else
				{
					...
					if (isArray && lexer.token() != JSONToken.LBRACKET)
					{
						...
						ObjectDeserializer derializer = config
								.getDeserializer(componentType);
			...
			if (lexer.token() != JSONToken.COMMA) { throw new JSONException(
					"syntax error :" + JSONToken.name(lexer.token())); }
	...
	public void parseObject(Object object)
	{
		...
		Map<String, FieldDeserializer> setters = config
		...
		for (;;)
		{
			...
			if (fieldDeser == null)
			{
				if (!isEnabled(Feature.IgnoreNotMatch)) { throw new JSONException(
			...
			else
			{
				...
				if (fieldClass == int.class)
				{
					...
					fieldValue = IntegerCodec.instance.deserialze(this,
							fieldType, null);
				}
				...
				{
					lexer.nextTokenWithColon(JSONToken.LITERAL_INT);
					fieldValue = LongCodec.instance.deserialze(this, fieldType,
							null);
				}
				else
				{
					ObjectDeserializer fieldValueDeserializer = config
							.getDeserializer(fieldClass, fieldType);

					lexer.nextTokenWithColon(fieldValueDeserializer
							.getFastMatchToken());
					fieldValue = fieldValueDeserializer.deserialze(this,
							fieldType, null);
				}
	...
	public Object parseArrayWithType(Type collectionType)
	{
		...
		Type[] actualTypes = ((ParameterizedType) collectionType)
				.getActualTypeArguments();

		if (actualTypes.length != 1) { throw new JSONException(
				"not support type " + collectionType); }
		...
		if (actualTypeArgument instanceof TypeVariable)
		{
			...
			if (bounds.length != 1) { throw new JSONException("not support : "
					+ typeVariable); }
	...
	public Object getObject(String path)
	{
		...
		{
			if (path.equals(contextArray[i].getPath())) { return contextArray[i]
					.getObject(); }
		}
	...
	public void checkListResolve(Collection array)
	{
		if (resolveStatus == NeedToResolve)
		{
			if (array instanceof List)
			{
				...
				task.setFieldDeserializer(new ListResolveFieldDeserializer(
						this, list, index));
			...
			{
				ResolveTask task = getLastResolveTask();
				task.setFieldDeserializer(new CollectionResolveFieldDeserializer(
						this, array));
				task.setOwnerContext(context);
				setResolveStatus(DefaultJSONParser.NONE);
			}
	...
	public void checkMapResolve(Map object, String fieldName)
	{
		if (resolveStatus == NeedToResolve)
		{
			MapResolveFieldDeserializer fieldResolver = new MapResolveFieldDeserializer(
					object, fieldName);
			ResolveTask task = getLastResolveTask();
	...
	public final void parseArray(final Collection array, Object fieldName)
	{
		...
		if (lexer.token() != JSONToken.LBRACKET) { throw new JSONException(
				"syntax error, expect [, actual "
						+ JSONToken.name(lexer.token()) + ", pos "
						+ lexer.pos()); }
		...
		try
		{
			for (int i = 0;; ++i)
			{
				...
				case LITERAL_STRING:
					...
					if (lexer.isEnabled(Feature.AllowISO8601DateFormat))
					{
						JSONScanner iso8601Lexer = new JSONScanner(
								stringLiteral);
						if (iso8601Lexer.scanISO8601DateIfMatch())
						{
							value = iso8601Lexer.getCalendar().getTime();
	...
	{
		if (isEnabled(Feature.DisableCircularReferenceDetect)) { return; }
	...
	{
		if (isEnabled(Feature.DisableCircularReferenceDetect)) { return null; }
	...
	public Object parse(Object fieldName)
	{
		...
		case LITERAL_STRING:
			...
			if (lexer.isEnabled(Feature.AllowISO8601DateFormat))
			{
				...
				try
				{
					if (iso8601Lexer.scanISO8601DateIfMatch()) { return iso8601Lexer
							.getCalendar().getTime(); }
				}
				finally
				{
		...
		case NEW:
			...
			if (lexer.token() != JSONToken.IDENTIFIER) { throw new JSONException(
					"syntax error"); }
		...
		case EOF:
			...
			throw new JSONException("unterminated json string, pos "
					+ lexer.getBufferPosition());
		case ERROR:
		default:
			throw new JSONException("syntax error, pos "
					+ lexer.getBufferPosition());
		}
	}
	...
	public void close()
	{
		...
		try
		{
			...
			{
				if (lexer.token() != JSONToken.EOF) { throw new JSONException(
	...
	{
		if (resolveTaskList == null) { return; }
