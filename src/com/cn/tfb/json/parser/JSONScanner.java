...
import com.cn.tfb.json.util.Base64;
...
public final class JSONScanner extends JSONLexerBase
{
	...
	{
		if (index >= text.length()) { return EOI; }
	...
	public final String addSymbol(int offset, int len, int hash,
	...
	// public int scanField2(char[] fieldName, Object object, FieldDeserializer
	// fieldDeserializer) {
	// return NOT_MATCH;
	// }
	...
	public boolean scanISO8601DateIfMatch(boolean strict)
	{
		...
		if (rest == 8 || rest == 14 || rest == 17)
		{
			...
			if (!checkDate(y0, y1, y2, y3, M0, M1, d0, d1)) { return false; }
			...
			if (rest != 8)
			{
				...
				if (!checkTime(h0, h1, m0, m1, s0, s1)) { return false; }
				...
				if (rest == 17)
				{
					...
					if (S0 < '0' || S0 > '9') { return false; }
					if (S1 < '0' || S1 > '9') { return false; }
					if (S2 < '0' || S2 > '9') { return false; }
		...
		if (rest < ISO8601_LEN_0) { return false; }

		if (charAt(bp + 4) != '-') { return false; }
		if (charAt(bp + 7) != '-') { return false; }
		...
		if (t == 'T' || (t == ' ' && !strict))
		{
			if (rest < ISO8601_LEN_1) { return false; }
		}
		else if (t == '"' || t == EOI)
		{
		...
		if (charAt(bp + 13) != ':') { return false; }
		if (charAt(bp + 16) != ':') { return false; }
		...
		if (dot == '.')
		{
			if (rest < ISO8601_LEN_2) { return false; }
		}
		else
		{
			calendar.set(Calendar.MILLISECOND, 0);
	...
	private boolean checkTime(char h0, char h1, char m0, char m1, char s0,
			char s1)
	{
		if (h0 == '0')
		{
	...
	private void setCalendar(char y0, char y1, char y2, char y3, char M0,
			char M1, char d0, char d1)
	{
		Locale local = Locale.getDefault();
		calendar = Calendar.getInstance(TimeZone.getDefault(), local);
	...
	static boolean checkDate(char y0, char y1, char y2, char y3, char M0,
			char M1, int d0, int d1)
	{
		if (y0 != '1' && y0 != '2') { return false; }
		if (y1 < '0' || y1 > '9') { return false; }
		if (y2 < '0' || y2 > '9') { return false; }
		if (y3 < '0' || y3 > '9') { return false; }

		if (M0 == '0')
		{
			if (M1 < '1' || M1 > '9') { return false; }
		}
		else if (M0 == '1')
		{
			if (M1 != '0' && M1 != '1' && M1 != '2') { return false; }
		}
		else
		{
			return false;
	...
	protected final void arrayCopy(int srcPos, char[] dest, int destPos,
			int length)
	{
