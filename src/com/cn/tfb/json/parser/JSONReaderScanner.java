...
public final class JSONReaderScanner extends JSONLexerBase
{
	...
	public final char charAt(int index)
	{
		if (index >= bufLength)
		{
			...
			if (bufLength == 0) { throw new JSONException(
					"illegal stat, textLength is zero"); }

			if (bufLength == -1) { return EOI; }
	...
	public final int indexOf(char ch, int startIndex)
	{
		...
		for (;; ++offset)
		{
			...
			if (ch == charAt(index)) { return offset + bp; }
			if (ch == EOI) { return -1; }
		}
	}
	...
	public final String stringVal()
	{
		if (!hasSpecial)
		{
			...
			if (offset > buf.length - sp) { throw new IllegalStateException(); }
	...
	{
		return bufLength == -1 || bp == buf.length || ch == EOI
				&& bp + 1 == buf.length;
	}
}
