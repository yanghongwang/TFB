...
public class CountQuery<T> extends AbstractQuery<T>
{
	...
	private final static class QueryData<T2> extends
			AbstractQueryData<T2, CountQuery<T2>>
	{
	...
	static <T2> CountQuery<T2> create(AbstractDao<T2, ?> dao, String sql,
	...
	private CountQuery(QueryData<T> queryData, AbstractDao<T, ?> dao,
	...
	public long count()
	{
		...
		try
		{
			...
			{
				throw new DaoException("Unexpected row count: "
						+ cursor.getCount());
			}
			else if (cursor.getColumnCount() != 1) { throw new DaoException(
					"Unexpected column count: " + cursor.getColumnCount()); }
