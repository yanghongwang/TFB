...
import com.cn.tfb.dao.DaoLog;
import com.cn.tfb.dao.query.Query;
...
class AsyncOperationExecutor implements Runnable, Handler.Callback
{
		...
		try
		{
			try
			{
				while (true)
				{
					...
					{
						AsyncOperation operation2 = queue.poll(
								waitForMergeMillis, TimeUnit.MILLISECONDS);
						if (operation2 != null)
						{
			...
			{
				DaoLog.w(
						Thread.currentThread().getName() + " was interruppted",
						e);
			}
		}
		finally
	...
	private void mergeTxAndExecute(AsyncOperation operation1,
			AsyncOperation operation2)
	{
		...
		try
		{
			for (int i = 0; i < mergedOps.size(); i++)
			{
				...
				{
					// Operation may still have changed the DB, roll back
					// everything
					failed = true;
					break;
				}
				if (i == mergedOps.size() - 1)
				{
					...
					if (i < maxOperationCountToMerge
							&& operation.isMergeableWith(peekedOp))
					{
						AsyncOperation removedOp = queue.remove();
						if (removedOp != peekedOp)
						{
							// Paranoia check, should not occur unless threading
							// is broken
							throw new DaoException(
	...
	private void executeOperation(AsyncOperation operation)
	{
		...
		try
		{
			...
			case DeleteInTxIterable:
				operation.dao
						.deleteInTx((Iterable<Object>) operation.parameter);
				break;
			...
			case InsertInTxIterable:
				operation.dao
						.insertInTx((Iterable<Object>) operation.parameter);
				break;
			...
			case InsertOrReplaceInTxIterable:
				operation.dao
						.insertOrReplaceInTx((Iterable<Object>) operation.parameter);
				break;
			case InsertOrReplaceInTxArray:
				operation.dao
						.insertOrReplaceInTx((Object[]) operation.parameter);
				break;
			case Update:
			...
			case UpdateInTxIterable:
				operation.dao
						.updateInTx((Iterable<Object>) operation.parameter);
				break;
